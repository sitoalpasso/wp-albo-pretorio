...
| A-31 | fatto | **Postcondizione degli elenchi di voci**: si controlla l'esito delle due registrazioni e si rilegge il registro prima di considerare il tipo registrato | il tipo risulta registrato **solo se** entrambi gli elenchi risultano registrati e agganciati al tipo. **Quando questa postcondizione cade il tipo è già registrato e non si può smontare in modo pulito**: cosa resta e cosa è comunque garantito sta in A-43 |
...
| A-39 | da fare | **Postcondizione dei permessi su ogni ruolo toccato**, non solo su quelli dichiarati: si sabota la scrittura di un permesso nuovo sul **solo ruolo completato automaticamente** | l'assegnazione fallisce, l'installazione non dichiara successo, la versione riletta resta la precedente, e alla richiesta successiva il lavoro viene ritentato. Verificare i soli ruoli dichiarati lascia scoperti proprio quelli che il componente completa da sé |
| A-40 | da fare | **Il permesso marcatore del ruolo non viene scritto**: si sabota la sua persistenza mentre il ruolo viene creato | nessun successo e nessuna versione memorizzata. **E c'è una via di recupero**: il ruolo appena creato senza marcatore viene rimosso, e la rimozione si rilegge. Senza recupero quel ruolo diventerebbe una collisione permanente, perché alla richiesta dopo il componente lo troverebbe senza marcatore e lo direbbe di altri, per sempre. Se anche la rimozione non riesce, l'avviso nomina il ruolo da togliere a mano |
| A-41 | da fare | **La proprietà si rilegge dallo stato corrente**: dopo una registrazione riuscita, un componente esterno registra lo stesso tipo oppure uno dei due elenchi di voci | da quel momento non sono più nostri: l'installazione non prosegue, nessun permesso viene assegnato, nessuna versione memorizzata, e lo sbarramento non tocca i contenuti di quel tipo. Un valore memorizzato dice che abbiamo registrato qualcosa in passato, non che l'oggetto nel registro sia ancora il nostro: WordPress lo sostituisce senza dire niente |
| A-42 | da fare | **Il segno nell'indirizzo di ritorno nasce solo da un invio riconosciuto**, in due direzioni: invio vero della schermata con il dato temporaneo sparito, e chiamata da codice durante una richiesta di amministrazione | nel primo caso il segno c'è lo stesso e la schermata mostra l'avviso generico; nel secondo **non c'è né il dettaglio né il segno**. Segnare prima di distinguere farebbe comparire a una persona un avviso per un salvataggio che non ha chiesto |
| A-43 | da fare | **Stato parziale sicuro** dopo la postcondizione mancata degli elenchi di voci (A-31): si descrive e si verifica **cosa resta registrato** | il tipo **resta registrato**, e uno dei due elenchi può restare: non esiste un modo pulito di smontare quel solo tipo. Quello che si garantisce è l'elenco chiuso di ciò che **non** succede: nessun permesso su nessun ruolo, nessuna versione memorizzata, lo sbarramento non tocca nessun contenuto, e un contenuto di quel tipo portato a pubblicato **resta irraggiungibile**, perché gli argomenti di registrazione sono quelli chiusi. **Non si dice "inerte" senza dire cosa resta** |
