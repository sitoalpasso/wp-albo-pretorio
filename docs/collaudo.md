...
| A-43 | fatto | **Stato parziale sicuro** dopo la postcondizione mancata degli elenchi di voci (A-31): si descrive e si verifica **cosa resta registrato** | il tipo **resta registrato**, e uno dei due elenchi può restare: non esiste un modo pulito di smontare quel solo tipo. Quello che si garantisce è l'elenco chiuso di ciò che **non** succede: nessun permesso su nessun ruolo, nessuna versione memorizzata, lo sbarramento non tocca nessun contenuto, e un contenuto di quel tipo portato a pubblicato **resta irraggiungibile**, perché gli argomenti di registrazione sono quelli chiusi. **Non si dice "inerte" senza dire cosa resta** |
