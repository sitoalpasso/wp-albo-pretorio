...
| ALBO-12 | da fare | Sostituzione per oscuramento su un atto **ancora esposto**, avendo prima dimostrato che il file uscente era scaricabile dal pubblico; poi si rileggono le tracce e si prova a scaricare | restano registrate **le impronte di tutte e due le versioni**, quella uscente e quella entrante, con chi l'ha disposta e quando (ALBO-10). Il pubblico ora **scarica il file entrante**, e quello che scarica corrisponde all'impronta registrata come entrante; il file uscente **non è più raggiungibile** da nessun percorso dell'elenco chiuso. Le tre verifiche servono insieme: senza la prova che il file uscente fosse pubblico prima, e senza quella che l'entrante lo sia dopo, la riga sarebbe verde anche chiudendo ogni scaricamento, o eseguendola su un atto già scaduto, dove nessun file è raggiungibile comunque. Senza le due impronte il referto non potrebbe dire quale file era esposto in quale periodo (ALBO-07) |
| ALBO-12 | da fare | [attacco] Due tentativi **indipendenti** di sostituzione per oscuramento, che cambiano una condizione per volta: chi non possiede il permesso di pubblicare, su un atto **pubblicato e non scaduto**; poi chi pubblica, su un atto **annullato** | rifiutata in tutti e due i casi, e l'allegato resta quello di prima. I due casi vanno tenuti separati: cumulare utente senza permesso e atto annullato in un tentativo solo lascerebbe verde la riga anche con una guardia sola delle due. **Controllo positivo**: chi pubblica la compie su un atto pubblicato e non scaduto, altrimenti la riga sarebbe vera anche con una funzione che non esiste |
...
| ALBO-27 | da fare | Si pubblica un atto e si rilegge la **data di inizio**; poi si ripete fornendo una data **passata**, esplicita, sia già scritta nella bozza sia inviata al momento della pubblicazione, da ogni ingresso supportato | è sempre quella dell'istante in cui la transizione è avvenuta, confrontata con l'orologio, e la data fornita **non diventa mai** quella effettiva. Il caso della data passata serve quanto quello della data futura: senza di esso un'implementazione che accetta il valore del chiamante e ricorre all'orologio solo quando manca supererebbe tutte e tre le righe di ALBO-27, perché la riga precedente prova solo il futuro e quella seguente solo gli atti già pubblicati. **Controllo positivo**: la pubblicazione senza data fornita riesce e la data viene assegnata dal sistema. Da questa data decorrono termini di legge, quindi una data scelta a mano sarebbe una dichiarazione falsa e non una comodità |
| ALBO-27 | da fare | [attacco] Si tenta di cambiare la data di inizio di un atto **pubblicato**, da ogni ingresso e per ogni ruolo, **amministratore compreso**; poi, separatamente, si invia una richiesta di **sostituzione per oscuramento** che porta con sé anche una data di inizio diversa e altri dati di scheda | nel primo caso sempre bloccato, come ogni altra modifica di un atto pubblicato (ALBO-09). Nel secondo la richiesta **non cambia nessun dato estraneo all'eccezione**: l'oscuramento è l'unica cosa che passa, e il catalogo esige che il comportamento sia dichiarato, cioè o la richiesta è rifiutata per intero o i campi estranei sono ignorati, purché non finiscano mai nell'atto. Il secondo caso serve perché il primo è già soddisfatto dal blocco generale di ALBO-09 e non dice niente sull'unico percorso di scrittura rimasto aperto su un atto pubblicato. **Controllo positivo**: la sostituzione per solo oscuramento, senza dati estranei, riesce |
