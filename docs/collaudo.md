...
| ALBO-01 | da fare | Si prova a **pubblicare** senza uno dei dati necessari, uno per volta | bloccato ogni volta, con il nome del dato che manca. **Due dati restano fuori dall'elenco perché li scrive il sistema**: il numero di repertorio e la data di inizio pubblicazione (ALBO-27). Chiederli al compilatore renderebbe atteso un rifiuto proprio nel caso corretto. **Controllo positivo**: un atto a cui la data di inizio non è mai stata fornita passa in verifica e si pubblica, e la data compare solo alla pubblicazione |
...
| ALBO-04 | da fare | Si configura un tipo di atto indicando **solo il numero di giorni**, senza dire se la durata venga da una norma o da una scelta dell'amministrazione; poi si ripete dichiarando l'origine **norma** senza i suoi estremi | rifiutata in tutti e due i casi, e il tipo resta senza durata valida, quindi i suoi atti non si pubblicano. L'origine è obbligatoria quanto il numero di giorni, perché è ciò che distingue un termine che non si accorcia da un massimale che l'amministrazione valuta caso per caso. **Controllo positivo**: le due configurazioni complete, una di origine normativa con gli estremi e una di origine dell'amministrazione, sono accettate, altrimenti la riga sarebbe soddisfatta rifiutando ogni configurazione |
| ALBO-04 | da fare | Su un atto il cui tipo ha durata di **origine normativa**, chi pubblica prova a indicare una data di fine più vicina di quella che la durata comporta; poi la stessa prova su un atto il cui tipo ha durata di **origine dell'amministrazione** | sul primo rifiutata, con la data di fine che resta quella calcolata: un termine prescritto da una norma non si accorcia. Sul secondo riesce, la scelta più breve vale, e **la motivazione finisce nel registro** insieme a chi l'ha disposta (ALBO-10). Le due prove servono insieme: da sola la prima sarebbe verde anche per un'implementazione che rifiuta sempre, da sola la seconda anche per una che accetta sempre. Su tutti e due i tipi resta rifiutata una data di fine **più lontana** di quella che la durata comporta, perché allungare non è la valutazione che la fonte chiede |
...
| ALBO-22 | da fare | Si manda in verifica una bozza a cui manca un dato necessario, uno per volta | bloccato ogni volta, con il nome del dato che manca. I dati richiesti sono quelli che il compilatore fornisce, **data di fine compresa e data di inizio esclusa**: l'inizio lo scrive il sistema alla pubblicazione (ALBO-27), quindi la sua assenza in bozza è lo stato corretto e non una mancanza. **Controllo positivo**: con tutti i dati, e senza data di inizio, la bozza passa in verifica, altrimenti la riga sarebbe soddisfatta rifiutando tutto |
...
della deliberazione, offre un argomento interpretativo ma non disciplina gli effetti di
